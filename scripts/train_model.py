...
def main() -> None:
    ...
    data_hash = sha256(
        pd.util.hash_pandas_object(feature_frame, index=True).values.tobytes()
    ).hexdigest()
    ...
    metadata = {
        ...
        "metrics": {"train_accuracy": round(float(pipeline.score(feature_frame, target)), 4)},
        "dependencies": {
            "joblib": joblib.__version__,
            "pandas": pd.__version__,
            "scikit-learn": sklearn.__version__,
        },
        "example_input": {name: float(feature_frame.iloc[0][name]) for name in FEATURE_NAMES},
    }
